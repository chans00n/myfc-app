-- Add parent_id to messages table for nested replies if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Add vote_count to messages table if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
